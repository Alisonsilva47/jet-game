...
class Jet(object):
    health = 3
    animation_number = 0
    animation_list = sprites.jet_list
    counter = 0
    current = sprites.jet_list[animation_number]
    crash_counter = 0
    damaged_counter = 0
    damaged = False
    wreck_start = False
    ...
    def wreck(self):
        ...
        if 5 <= self.crash_counter < 10:
            self.current = sprites.explosion_0
        if 10 <= self.crash_counter < 15:
            self.current = sprites.explosion_1
        if 15 <= self.crash_counter < 20:
            self.current = sprites.explosion_2
        if 20 <= self.crash_counter < 25:
            self.current = sprites.explosion_3
        if self.crash_counter >= 20:
            self.current = sprites.explosion_7
    ...
    def blink_red(self):
        ...
        if self.damaged_counter >= 15:
            self.animation_list = sprites.jet_list
            self.damaged = False
